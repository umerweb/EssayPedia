      ...
      <div className='mt-12 flex flex-wrap gap-8'>
        ...
        {categories.map((category) => (
          ...
          <div  className="flex p-4 bg-white justify-start items-center gap-4 shadow-md hover:shadow-lg transform hover:scale-105 transition-transform duration-300">
            ...
            <h2 className='text-slate-700  text-md'>{category.cat || <Skeleton count={8} width={200} height={200} />}</h2>
          </div></Link>
        ))}
      </div>
    </div>
      ...
      {newpost.map(post => (
          <Link key={post._id} to={`/posts/${post._id}`}>
            <div className="bg-white p-4  rounded-md  shadow-md transition duration-300 hover:shadow-lg">
              <div className="mb-4">
                <h2 className="md:text-lg text-md font-bold text-gray-800 hover:text-blue-600">{post.title || <Skeleton />}</h2>
                <p className="text-sm text-gray-600 mb-2 italic font-semibold">{post.uni || <Skeleton />}</p>
                <p className="text-sm text-gray-600"><b>Category: </b>{post.category || <Skeleton />}</p>
              </div>
            </div>
          </Link>
        ))}
